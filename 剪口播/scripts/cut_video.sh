...
# 选择编码参数。DJI 等相机常见 HEVC Main 10 / yuv420p10le，
# 不能用 H.264 high profile 编码，否则 x264 会报 10-bit 不支持。
PROFILE_LC=$(echo "$PROFILE" | tr '[:upper:]' '[:lower:]')
if [[ "$PIX_FMT" == *"10"* ]] || [[ "$PROFILE_LC" == *"10"* ]]; then
  if ffmpeg -hide_banner -encoders 2>/dev/null | grep -q "hevc_videotoolbox"; then
    VIDEO_CODEC="hevc_videotoolbox"
    VIDEO_CODEC_ARGS="-c:v hevc_videotoolbox -profile:v main10 -tag:v hvc1 -b:v ${BITRATE_K}k -maxrate ${MAXRATE_K}k -bufsize ${BUFSIZE_K}k -pix_fmt $PIX_FMT"
  else
    VIDEO_CODEC="libx265"
    VIDEO_CODEC_ARGS="-c:v libx265 -profile:v main10 -tag:v hvc1 -b:v ${BITRATE_K}k -maxrate ${MAXRATE_K}k -bufsize ${BUFSIZE_K}k -pix_fmt $PIX_FMT"
  fi
else
  case "$PROFILE_LC" in
  ...
  VIDEO_CODEC="libx264"
  VIDEO_CODEC_ARGS="-c:v libx264 -profile:v $X264_PROFILE -b:v ${BITRATE_K}k -maxrate ${MAXRATE_K}k -bufsize ${BUFSIZE_K}k -pix_fmt $PIX_FMT"
fi
...
echo "   编码: $VIDEO_CODEC -b:v ${BITRATE_K}k -pix_fmt $PIX_FMT"
